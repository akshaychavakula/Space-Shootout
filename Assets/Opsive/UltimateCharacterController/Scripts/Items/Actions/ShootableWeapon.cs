...
using Opsive.UltimateCharacterController.Networking.Game;
    ...
    public class ShootableWeapon : UsableItem, IReloadableItem
    {
            ...
            if (!m_ReloadInitialized) {
                ...
                if (m_SharedConsumableItemIdentifier && (itemSetManager = m_Inventory.GetComponent<ItemSetManagerBase>()) != null) {
                    for (int i = 0; i < m_Inventory.SlotCount; ++i) {
                        ...
                        var item = m_Inventory.GetItem(i, itemIdentifier);
        ...
        public bool CanReloadItem(bool sharedCheck)
        {
            ...
            // Reload also should not start if there's not enough consumable amount for the newly added item and the current item.
            if (sharedCheck && m_SharedConsumableItemIdentifier) {
                if (m_Inventory.GetActiveItem(m_Item.SlotID) != m_Item) {
                    return false;
                }
                if (m_Inventory.GetItemIdentifierAmount(m_ConsumableItemIdentifier) < m_ClipSize) {
                    return false;
                }
            }
        ...
        public void ReloadItem(bool fullClip)
        {
            ...
            if (m_ActiveSharedConsumableItemIdentifierAmount > 0) {
                ...
                if (m_TotalReloadAmount > consumableItemIdentifierAmount) {
                    var totalAmount = consumableItemIdentifierAmount + m_TotalClipAmount;
                    ...
                    var sharedAmount = Mathf.Min(consumableItemIdentifierAmount,
                                        (fullClip || m_ReloadType == ReloadClipType.Full) ? (int)(totalAmount / (m_ActiveSharedConsumableItemIdentifierAmount + 1)) : 1);
                    var sharedDiff = sharedAmount - ClipRemaining;
                    if (sharedDiff < sharedAmount) {
                        sharedDiff = Mathf.FloorToInt(sharedDiff / (m_ActiveSharedConsumableItemIdentifierAmount + 1));
                    }
                    reloadAmount = Mathf.Min(m_ClipSize - ClipRemaining, sharedDiff);
                } else {
                    // The Consumable ItemIdentifier doesn't need to be shared if there is plenty of ammo for all weapons.
