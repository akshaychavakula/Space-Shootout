...
#endif
                    var count = m_Inventory.GetItemIdentifierAmount(m_ItemIdentifier);
                    if (count > 0) {
                    ...
                    if (count > 0 || forceDrop) {
                        ...
                        if (itemPickup != null) {
                            ...
                            for (int j = 0; j < itemDefinitionAmounts.Length; ++j) {
                                GenericObjectPool.Return(itemDefinitionAmounts[j]);
                            }
                            var itemDefinitionAmount = GenericObjectPool.Get<ItemDefinitionAmount>();
                            itemDefinitionAmount.Initialize(m_ItemIdentifier.GetItemDefinition(), 1);
                            ...
                            for (int i = 0; i < m_ItemActions.Length; ++i) {
                                if ((usableItem = (m_ItemActions[i] as UsableItem)) != null && usableItem.GetConsumableItemIdentifier() != null) {
