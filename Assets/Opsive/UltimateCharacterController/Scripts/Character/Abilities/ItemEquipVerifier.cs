    ...
    public class ItemEquipVerifier : Ability
    {
        ...
        public bool TryToggleItem(Ability ability, bool activate)
        {
            ...
            if (ability == m_StartingAbility && IsActive && !activate) {
                ...
                return true;
            } else if (m_StartingAbility != null && ((!activate && m_StartingAbility != ability) || (activate && m_StartingAbility == ability))) {
                // Do not change the equip status if the ability is stopping and is not the starting ability, or the ability is already active.
                return false;
            }
