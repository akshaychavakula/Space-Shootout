...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public abstract class InventoryBase : MonoBehaviour
    {
            ...
            if (notifyOnPickup) {
                if (slotID == -1) {
                    ...
                    for (int i = 0; i < m_SlotCount; ++i) {
                        if (GetItem(i, itemIdentifier) != null) {
                ...
                if (slotID != -1) {
                    var item = GetItem(slotID, itemIdentifier);
        ...
        public Item GetItem(int slotID, IItemIdentifier itemIdentifier) { return GetItemInternal(slotID, itemIdentifier); }

        /// <summary>
        /// Internal method which returns the item that corresponds to the specified IItemIdentifier.
        /// </summary>
        /// <param name="slotID">The ID of the slot which the item belongs to.</param>
        /// <param name="itemIdentifier">The IItemIdentifier of the item.</param>
        /// <returns>The item which occupies the specified slot. Can be null.</returns>
        protected abstract Item GetItemInternal(int slotID, IItemIdentifier itemIdentifier);
        ...
        public void RemoveItem(IItemIdentifier itemIdentifier, int slotID, bool drop)
        {
            var item = GetItem(slotID, itemIdentifier);
            if (item != null) {
                    ...
                    for (int i = 0; i < itemActions.Length; ++i) {
                        if (((usableItem = itemActions[i] as IUsableItem) != null) && (consumableItemIdentifier = usableItem.GetConsumableItemIdentifier()) != null) {
                            ...
                            if (GetItemIdentifierAmount(itemIdentifier) == 1) {
                                RemoveItemIdentifierInternal(consumableItemIdentifier, slotID);
                            ...
                            var consumableCount = GetItemIdentifierAmount(consumableItemIdentifier);
                            EventHandler.ExecuteEvent(m_GameObject, "OnInventoryAdjustItemIdentifierAmount", consumableItemIdentifier, consumableCount);
                            if (m_OnAdjustItemIdentifierAmountEvent != null) {
                                m_OnAdjustItemIdentifierAmountEvent.Invoke(consumableItemIdentifier, consumableCount);
                            }
            ...
            RemoveItemIdentifierInternal(itemIdentifier, slotID);
            if (GetItemIdentifierAmount(itemIdentifier) == 0) {
        ...
        /// <param name="itemIdentifier">The ItemIdentifier to remove.</param>
        /// <param name="slotID">The ID of the slot.</param>
        protected abstract void RemoveItemIdentifierInternal(IItemIdentifier itemIdentifier, int slotID);
        ...
        public void RemoveAllItems()
        {
            ...
            for (int i = allItems.Count - 1; i >= 0; --i) {
                ...
                while (GetItemIdentifierAmount(itemIdentifier) > 0) {
                    RemoveItem(itemIdentifier, slotID, true);
                }
            }
        }
