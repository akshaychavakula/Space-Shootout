...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public class Inventory : InventoryBase
    {
        ...
        {
            return item != null && GetItemInternal(item.SlotID, item.ItemIdentifier == null ? item.ItemDefinition.CreateItemIdentifier() : item.ItemIdentifier);
        }
        ...
        protected override bool AddItemInternal(Item item)
        {
            if (item.ItemIdentifier == null) {
                Debug.LogError($"Error: Item {item.gameObject.name} has no ItemIdentifier.");
                return false;
            }
            ...
            if (m_ItemIdentifierMap[item.SlotID].ContainsKey(item.ItemIdentifier)) {
                return false;
            }

            // The item doesn't exist - add it.
            m_ItemIdentifierMap[item.SlotID].Add(item.ItemIdentifier, item);

            // The item can be added without being picked up yet - add to the ItemIdentifierAmount so the item can safely be removed.
            if (!m_ItemIdentifierAmount.ContainsKey(item.ItemIdentifier)) {
                m_ItemIdentifierAmount.Add(item.ItemIdentifier, 0);
            }
            return true;
        }
        ...
        protected override void RemoveItemIdentifierInternal(IItemIdentifier itemIdentifier, int slotID)
        {
            if (!m_ItemIdentifierAmount.TryGetValue(itemIdentifier, out var existingAmount)) {
                return;
            }
            ...
            if (itemIdentifier is ItemType) {
                var itemType = itemIdentifier as ItemType;
                m_ItemIdentifierAmount[itemType] = Mathf.Clamp(existingAmount - 1, 0, itemType.Capacity);
            } else {
                m_ItemIdentifierAmount[itemIdentifier] = existingAmount - 1;
            }
